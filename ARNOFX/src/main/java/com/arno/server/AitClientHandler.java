...
import static com.arno.resources.AitCommandTypes.*;
...
public class AitClientHandler implements Runnable {
    ...
    public void run() {
        try {
            AitLogger.getInstance().logToConsole(new Object[] {
                    "com/arno/client " + clientData.getId() + " connecting..." });
        } catch (Exception e) {
            try {
                        ...
                        if(validLogin(login))
                        {
                            ...
                            if(validPassword(login, password)) {
                                ...
                                if (!clientData.getIsLoggedIn()) {
                                    ...
                                    AitLogger.getInstance().logToConsole(new Object[]{
                                            "com/arno/client " + clientData.getId() + " by " + clientData.getLogin() + " login success :)"});
                                }
                            }
                            else {
                                ...
                                AitLogger.getInstance().logToConsole(new Object[] { "com/arno/client " + data.getClient().getLogin() + " give wrong password!" });
                            }
                        }
                        break;
    ...
    private boolean validLogin(String login) throws Exception {
        if(AitLocalDB.getInstance().validLogin(login)) {
            ...
            AitLogger.getInstance().logToConsole(new Object[] { "com/arno/client " + data.getClient().getLogin() + " must reqister!" });
