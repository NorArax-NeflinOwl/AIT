...
import static com.hbm.resources.AitCommandTypes.ClientInitConnection;
...
import static com.hbm.resources.AitCommandTypes.SendFrom;
...
public class AitClientHandler implements Runnable {
    ...
    public void run() {
        try {
            System.out.println("com/ptl/client " + clientData.getId() + " connecting..." );
        } catch (Exception e) {
            try {
                        ...
                        if(validLogin(login))
                        {
                            ...
                            if(validPassword(login, password)) {
                                ...
                                if (!clientData.getIsLoggedIn()) {
                                    ...
                                    System.out.println("com/ptl/client " + clientData.getId() + " by " + clientData.getLogin() + " login success :)");
                                }
                            }
                            else {
                                ...
                                System.out.println("com/ptl/client " + data.getClient().getLogin() + " give wrong password!");
                            }
                        }
                        break;
    ...
    private boolean validPassword(String login, String password) {
        return true; //Objects.requireNonNull(AitLocalDB.getInstance()).validPass(login, password);
    }
